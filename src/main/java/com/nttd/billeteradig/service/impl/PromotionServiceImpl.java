...
public class PromotionServiceImpl  implements PromotionService {
    ...
    public Uni<ResponseDto> getTodayPromotion1(){
        ...
        return promotionRedis.get("promotions").map(obj->{
            if(obj != null){
                return new ResponseDto(Response.Status.OK.getStatusCode(), mensajeGeneral, obj);
            }
               
            return PromotionEntity.list("state",valorActivo).map(res->{
                 
                     promotionRedis.set("promotions", res.get(0));
                     return new ResponseDto(Response.Status.OK.getStatusCode(),
                                                     mensajeGeneral, res.get(0));
                });
        ...
        return promoactivo.map(res ->{
                ...
                }).onFailure().recoverWithItem(ex -> new ResponseDto(Response.Status.BAD_REQUEST.getStatusCode(), excepcionGeneral, ex.getMessage()));
            */   
